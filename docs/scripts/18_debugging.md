...
local function Inspect()
    ...
    if hit.entity ~= nil then
        Debug.Print("  Entity '" .. hit.entity.name .. "' id", hit.entity.id,
...
function Start()
    ...
    if camera == nil or pc == nil then
        Debug.LogError("DebugTools: " .. entity.name .. " needs a Camera and a PlayerController")
    end
end
...
local ok, result = pcall(function()
    return entity:GetTag(99)   -- there is no 99th tag
end)
...
- Prefer checking `isValid` (Entities, sectors, walls, floors, behaviours) or `nil`
